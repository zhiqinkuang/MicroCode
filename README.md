...
- **编辑—验证闭环**：改了文件却没拿到通过的验证时，系统会拦停收尾、强制补验证，失败输出持续回流。
- **斜杠命令**：
  - `/new` 开启新会话
...
- 模型偶尔会声称「没有收到图片」并要求给文件路径。根因不是链路：本地假服务抓到线载荷里图片块始终
  正确发出，裸调用同一模型 40 次也从不拒答；是「终端编程助手 + 文件工具」这个上下文让模型误判内联图片
  不存在。`agent/core.py` 的指令已明确说明图片随消息直接到达，实测产品路径拒答率从 **5/40 降到 0/60**。
...
Agent 的工具循环本身是多轮的，但「什么时候算干完」原本完全由模型自己说了算：写完代码直接回复
「已完成」也能结束这一轮。闭环补上的是**系统侧的收口**——不再依赖模型自觉。

机制（`agent/iteration.py` + `agent/hooks.py`）：

1. `edit_file` / `write_file` 写盘成功后，把文件路径记进本轮闭环状态。
2. 跑测试 / 构建 / 检查这类验证命令时，用 `run_command(verify=True)` 声明——退出码和输出尾部被记账。
3. 模型**想收尾**（回复里没有任何工具调用）但本轮改过文件、且最后一次验证没有通过时，
   系统抛出 `ModelRetry` 拒收这次回复，把验证要求灌回去：已改动的文件清单、上次验证命令、
   退出码、输出尾部，以及「改用 verify=True 再跑一次」的明确要求。
4. 模型据此继续修 → 再验证，形成「探索 → 修改 → 验证 → 纠错」的闭环。

拦停时终端会打印 `◈ verify` 一行，方便你看到系统何时介入、介入到第几次。

边界与上限：

- 最多拦停 **3 次**，之后放行并以「别再试了，如实向用户交代」收口——不会把一轮对话卡死。
  实测模型始终不验证时，正好跑 5 轮模型调用（1 轮写文件 + 3 轮被拦 + 1 轮收尾）后正常结束。
- 本轮没改任何文件（纯问答、纯查询）→ 零拦停，行为与以前一致。
- 模型还在调工具时绝不打扰：拦停判据是「回复里没有工具调用」，与 pydantic-ai 采纳 `End` 的条件一致。
- 闭环状态是**运行时状态**，不写进会话文件：`/resume` 恢复会话时重建为空，
  避免一恢复就被一条催验证提醒迎面砸中。
- 首版只覆盖主 agent；子代理不注入该状态，行为不变。

设计取舍与实测记录见 `docs/plans/2026-09-12-opt-loop-design.md`。
